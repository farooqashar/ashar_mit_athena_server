...
if __name__ == '__main__':
    def check_positive(value):
        ...
        return ivalue
    parser = argparse.ArgumentParser(description='6.033 MapReduce')
    parser.add_argument('filename', metavar='filename', type=str, help='file for reading')
    parser.add_argument('-m', '--maptask', metavar='maptask', type=check_positive, default=2, help='number of map tasks')
    parser.add_argument('-r', '--reducetask', metavar='reducetask', type=check_positive, default=2, help='number of reduce tasks')
    parser.add_argument('-c', '--crash', action='store_true', help='whether worker will crash')
    args = parser.parse_args()

    # Create a WordCount MapReduce program
    start = time.time()
    wc = WordCount(maptask=args.maptask, reducetask=args.reducetask, path=args.filename, crash=args.crash)
	# Run it
    wc.run()
    ...
    print("")
    print("Total time elapsed: ", stop - start, "seconds")
